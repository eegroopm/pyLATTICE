...
class TableWidget(QtGui.QTableWidget):
    ...
    def appendData(self, data):
        ...
        pandas dataframe
        """
        fn0, header1 = self.iteratorFn(data)
        if fn0 is None:
            self.clear()
            return
        it0 = fn0(data)
        try:
            first = next(it0) #updated for python3

        except StopIteration:
            return
        #if type(first) == type(np.float64(1)):
        #   return
        fn1, header0 = self.iteratorFn(first)
        if fn1 is None:
            self.clear()
            return
        ...
        if not self.horizontalHeadersSet and header1 is not None:
            ...
            self.setColumnCount(len(header1))
    ...
    def iteratorFn(self, data):
        ...
        elif isinstance(data,pd.core.frame.DataFrame): #added by EEG 2/3/2014
            return(self.iterPD,list(data.columns))
            #return(self.iterPD,None)
        
        elif isinstance(data, dict):
            return lambda d: d.itervalues(), list(map(str, data.keys()))
            
        elif HAVE_METAARRAY and isinstance(data, metaarray.MetaArray):
            if data.axisHasColumns(0):
                header = [str(data.columnName(0, i)) for i in list(range(data.shape[0]))]
    ...
    def iterPD(self,data):
        for i in list(data.index):
            yield data.loc[i].values
    ...
    def setRow(self, row, vals):
        if row > self.rowCount()-1:
            self.setRowCount(row+1)
        for col in list(range(self.columnCount())):
            val = vals[col]
